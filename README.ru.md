...
- **Идёт в интернет, когда корпус не отвечает** — `web_search` с тирами доверия
  (аудированный отчёт > офиц. IR/новостная лента > неизвестный блог) закрывает
  пробелы по компаниям и цифрам, которых нет в EDGAR, цитирует каждый веб-факт с
  доменом и уровнем доверия и **обогащает базу** найденным, чтобы тот же вопрос в
  следующий раз отвечался из БД — без повторного поиска.
- **Знает, когда остановиться** — число веб-поисков ограничено, а на заведомо
  недоступных данных (частная компания, прогноз будущего) агент честно об этом
  говорит, а не зацикливается.
...
|---|---|---|
| **Диалоговый UI** | Чат-интерфейс с живым нарратором («ищу в интернете…»), анимированный таймлайн плана/шагов, markdown-ответ, сводка стоимости/токенов, тёмная/светлая тема и EN/RU. | [живое демо](https://ledgerlens.space/app/) ![интерфейс чата](demo/screenshots/self_correction_worker.png) |
...
| **Самокоррекция** | Шаг, вернувший пусто, переигрывается и повторяется на виду — а заведомо отсутствующая цифра честно признаётся, не выдумывается. | ![replan](demo/screenshots/self_correction_replan.png) |
| **Цитаты** | Нарративные ответы несут ссылки `sec.gov` / MOEX на каждое утверждение. | ![цитаты](demo/screenshots/citations.png) |
| **Веб-поиск с тирами доверия** | Когда компании или цифры нет в корпусе, агент ищет в вебе, помечает доверие к каждому источнику и кэширует найденные факты, чтобы следующий прогон отвечал из БД. | [живое демо](https://ledgerlens.space/app/) ![веб-поиск](demo/screenshots/web_search.png) |
...
- **Подключаемые worker-ноды:** оркестратор общается с воркерами по A2A, так что
  вторая нода — одна запись в `config/workers.yaml` (T-031); один и тот же контракт
  локально и удалённо. Публичное демо гоняет один локальный воркер на
  воркстейшн-бэкенде.
...
- **Веб-поиск с доверием и обогащением:** инструмент `web_search` идёт в открытый
  веб (Tavily API / реестры) только когда SQL/RAG не отвечают, оценивает каждый
  источник тиром доверия, кросс-проверяет при отсутствии единого доверенного и
  пишет извлечённые факты в `web_facts` — чтобы повторы били в БД, а не в сеть.
...
- **Живое демо:** https://ledgerlens.space/app/ *(публичное, с лимитами по частоте
  и бюджету; воркстейшн-бэкенд через малый VPS — DNS-only + Let's Encrypt, без
  CDN-прокси в SSE-пути — может быть недоступно во время обслуживания).*
...
| [BACKLOG.md](BACKLOG.md) | Бэклог разработчика: задачи T-001…T-046 с ТЗ и критериями |

## Статус проекта

Гейты **G1 ✅ G2 ✅ G3 ✅**. Ключевые задачи T-001…T-034 и T-041 сделаны, и демо
теперь работает на текущей топологии — **бэкенд на воркстейшне** за тонкой
**VPS-дверью** (DNS-only + Let's Encrypt, без CDN-прокси в SSE-пути), **фронт на
GitHub Pages**.

Недавно добавлено поверх ядра:

- **Диалоговый UI демо** (T-042) — чат, живой нарратор, анимированный таймлайн
  плана/шагов, markdown-ответы, сводка стоимости/токенов, тёмная/светлая + EN/RU.
- **Веб-поиск с тирами доверия** (T-043) — закрывает пробелы, которых нет в
  корпусе, с доменной моделью доверия и долговременным кэшем.
- **Веб-обогащение БД** (T-045) — найденные в вебе факты складываются в
  `web_facts` и видны SQL-пути агента, так что повтор отвечается из БД, а не
  повторным поиском.
- **Робастность** (T-046) — ограниченное число поисков и fail-fast на заведомо
  недоступных данных: агент честно деградирует, а не зацикливается.
- **Уведомления владельцу** (T-044) — каждый прогон демо шлёт стоимость/токены в
  Telegram владельца (egress через VPS).

Релиз v1.0 (T-040 / G4) и часть железо-зависимых шагов (локальный инференс,
бенчмарки T-037) — остаются. Что осознанно вне скоупа или упирается в железо —
см. [Известные ограничения](#известные-ограничения) ниже.
...
- **Локальный инференс отложен.** Публичное демо работает целиком на облачном
  LLM-тире (DeepSeek) — с проводного канала воркстейшна он быстр и надёжен.
  Опциональный *локальный* тир — GPU (RX 6900XT через ROCm) на воркстейшне или
  CPU-инференс — пока не подключён (Phase 2 миграции). Local-CPU-часть
  [бенчмарка инференса](benchmarks/inference/REPORT.md) и выбор локальной модели
  (ADR-3) ждут эту настройку.
