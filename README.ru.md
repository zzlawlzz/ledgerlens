...
A2A-воркер и web (nginx); при пустой БД загружает тикеры из SEC EDGAR (с
диск-кэшем) вместе с эмбеддингами и прогоняет смоук: числовые вопросы,
...
Гейты **G1 ✅ G2 ✅**; ключевые задачи T-001…T-037 сделаны, G3 (две ноды) и G4
(релиз v1.0) в работе. Текущие известные ограничения:
