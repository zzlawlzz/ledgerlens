...
в работе, с остатком из живых/железо-зависимых шагов. Что осознанно вне скоупа
или ещё упирается в железо — см. [Известные ограничения](#известные-ограничения)
ниже.

## Известные ограничения

Честные текущие ограничения линейки v1.0. Ни одно не замалчивается — каждое либо
привязано к задаче/гейту, либо осознанно вынесено из скоупа.

- **`faithfulness` — `non_blocking` в eval-гейте**, пока не закрыт T-041 (синтез
  не должен дополнять retrieved-контекст). Метрика считается и репортится на
  quality-дашборде; пока просто не роняет CI.
- **Развёртывание на двух нодах ещё не живое** (T-031, гейт G3). Диспетчер
  (round-robin + local-preferred failover) и валидатор безопасного URL готовы;
  остаётся живой деплой удалённой ноды через AmneziaWG-меш.
- **Локальный CPU-инференс медленный.** На dev-ноутбуке мультишаговые вопросы
  занимают минуты; целевое железо — домашняя нода (2× EPYC). Local-CPU-часть
  [бенчмарка инференса](benchmarks/inference/REPORT.md) и выбор локальной модели
  (ADR-3) ждут это железо.
- **Покрытие RU-источников частичное (Q-02).** MOEX ISS живой (SBER / GAZP /
  LKOH) и доказывает pluggable-интерфейс адаптера; e-disclosure и ГИР БО остаются
  каркасами интерфейса, осознанно вне скоупа v1.0. Данные MOEX ISS используются
  только в информационно-ознакомительных целях — см.
  [Источники данных и лицензии](#источники-данных-и-лицензии).
- **Глубина истории цен ограничена бесплатным тиром.** Бесплатный ключ Alpha
  Vantage отдаёт лишь ~100 последних торговых дней (compact); более длинная
  история требует премиум-ключа. Цены кэшируются после первого запроса.
- **Бенчмарки исключают GPU (Q-07).** Сравнивается только локальный-CPU vs
  облачный-API инференс; vLLM / GPU-бенчмарк осознанно отложен.
- **Публичное демо ограничено по скоупу.** Отдаёт только корпус US / EDGAR и
  применяет per-IP лимиты частоты, конкурентности и дневной стоимости — это
  витрина, а не SLA-сервис.
