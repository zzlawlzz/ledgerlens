...
| T-015 | Compose v1, смоук-тест — **гейт G1** | 1 | T-006, T-011, T-014 | S | [wip 2026-07-10 19c8f63]⁴ |
...
⁴ T-015: инфраструктура готова и проверена вживую (docker compose up → контейнер healthy, миграции на старте, curl-вопрос через SSE отвечает числом из БД); приёмка гейта G1 требует живого `make demo-ingest` (5 тикеров) — ждёт снятия SEC-троттла, как и T-011.
...
³ T-011: код и тесты готовы (идемпотентность и изоляция ошибок проверены интеграционно на фейк-адаптере); живой критерий (3 тикера × 3 года, сверка выручки AAPL) временно заблокирован троттлингом SEC (после ~30 МБ скачанных 10-K оба хоста отвечают на скорости капли — зеркальный риск §5.6 подтверждён на практике). Перепрогнать `uv run pytest -m slow tests/integration/test_ingestion_live.py` после паузы и перевести в done.
