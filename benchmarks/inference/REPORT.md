...
- **Hardware:** Windows-11-10.0.22631-SP0; 24 logical CPUs. _(The local candidates run on the same workstation.)_
...
- **No GPU / vLLM column (Q-07).** GPU inference was excluded by owner decision; we do not publish anyone else's GPU throughput as our own. The local column is CPU-only (the project workstation), API column is the paid tier.
