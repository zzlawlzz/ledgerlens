...
- `[регулярная] T-041 робастность: rerank soft-timeout + RRF-fusion-fallback — tools/rag/core.py + config/rag.yaml + tests/unit — 2026-07-13 — В РАБОТЕ.` Причинный фикс Q-22-корня (медленный cross-encoder на голодном CPU съедает worker `deadline_s` → шаг рвётся в середине rerank → 0 цитат). Добавляю config-gated `search.rerank.timeout_s`: при таймауте rerank откатываемся на уже полученный RRF-fusion-порядок (цитаты есть, а не пусто) + освобождаем бюджет под синтез. **Default off = поведение байт-в-байт как было** (dev/eval не тронуты). НЕ трогаю пороги гейта / eval.yml / eval-thresholds.yaml. Юнит-тесты на fallback-механизм локально. Живое включение+валидация на EPYC-раннере = остаток (домен оркестратора, тот же runner-блокер).
