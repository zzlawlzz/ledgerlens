...
4. **T-031 (гейт G3, критический путь) — `[wip]`, задел готов:** решение —
   ...
   **ОСТАЛОСЬ:** (1) диспетчер — маршрутизация+failover (см. Q-20: round-robin
   среди skill-воркеров + failover local; дефолт применяю); (2) живой деплой
   на 104.238.24.196 + WireGuard на домашней ноде (аккуратно, не рушить demo-стек);
   (3) живые критерии G3. Нода готова (Q-18), ключ id_ed25519_worker_node.
