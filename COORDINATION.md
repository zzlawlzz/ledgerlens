...
- `[регулярная] T-036 §1 defect-fix #2: spoofable demo rate-limit (X-Forwarded-For) — orchestrator/api.py (_client_ip) + tests/unit/test_demo_limits.py — старт ~сейчас — WIP.` Реальный release-critical security-баг public-demo: `_client_ip` доверяет ЛЕВОМУ значению `X-Forwarded-For` (клиент-контролируемое). В топологии демо nginx НЕ делает `$proxy_add_x_forwarded_for` → клиент-присланный XFF проходит Cloudflare+nginx к app нетронутым → ЛЮБАЯ позиция XFF спуфится → посетитель ротацией заголовка получает свежий rate-bucket на запрос → обходит budget-exhaustion-защиту (весь смысл T-036 §1). Фикс: доверять только `CF-Connecting-IP` (Cloudflare переписывает, единственный ingress=туннель → неспуфабельно), иначе прямой peer; XFF не доверяется. Хотспот `api.py` — коммичу сразу.
