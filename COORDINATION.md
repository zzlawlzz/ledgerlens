...
- `[регулярная] T-036 §1 defect-fix #3: /agui concurrency-slot leak (overlong thread_id) — orchestrator/api.py (хотспот) + tests — В РАБОТЕ (старт ~сейчас).` Реальный public-demo availability-баг: `/agui` берёт concurrency-слот в `_admit`, затем `ChatRequest(session_id=body.thread_id)` (line 440) ВНЕ slot-release-guard, а `session_id` имеет `max_length=128`, тогда как AG-UI `thread_id` не ограничен → клиент с `thread_id`>128 симв. вешает ValidationError после захвата слота → слот течёт навсегда + run осиротевает в `running`. На демо (cap=2) два таких запроса намертво вешают демо на «busy» 429. Трогаю ТОЛЬКО `orchestrator/api.py` (коммичу сразу) + `tests/unit/`. НЕ трогаю demo_limits.py/пороги/prompts/graph/eval/compose/nginx.
