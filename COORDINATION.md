...
- `[регулярная] T-040 release-hygiene: стейл-стаб `make eval` (TODO-заглушка при готовом T-029-харнессе) — Makefile — старт ~сейчас — WIP.` Тронет ТОЛЬКО `Makefile` (не хотспот) + BACKLOG-сноска + COORDINATION. НЕ трогает prompts/config/compose/api.py/graph/worker/eval.yml/web.
