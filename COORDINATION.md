...
- `[регулярная] DoD §2 остаток: SSE keep-alive на шаге >CF-idle-timeout (тяжёлый multi-step запрос над публичным демо) — docs/release/v1.0-dod.md (docs-only) — 2026-07-14 ~+18ч — В РАБОТЕ.` Инфра live-сверена: демо `app.ledgerlens.space`=200, сайт=200, раннер `epyc-home`=online+idle (eval НЕ идёт → окно чистое, демо-стек трогать безопасно). Анонимный read-only `/api/chat` над ПУБЛИЧНЫМ демо (тот же approved-паттерн, что +12ч сессия). НЕ трогаю prompts/config/compose/api.py/graph/worker/eval.yml/web.
...
§5 eval-в-CI теперь чист — citation-гейт разблокирован).
