...
- `[регулярная] диагностика/пуш eval-блокера — git push + re-dispatch, COORDINATION.md — 06:05 — ЗАВЕРШЕНО.`
- `[регулярная] T-036 §2 (make seed / make snapshot) — Makefile, scripts/eval_snapshot.py (+доп. флаг), scripts/fetch_demo_snapshot.sh, deploy/demo/ — 06:10 — В РАБОТЕ.` НЕ трогаю api.py/app.yaml/graph.py/worker_client.py/prompts/. Тестирую restore в scratch-БД, НЕ рушу общий стек (`down -v` не делаю).
