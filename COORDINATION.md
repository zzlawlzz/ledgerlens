...
- `[регулярная] T-041 citation-блокер: фикс CPU-голодания GH-раннера (Q-22 е+ж) — ЗАВЕРШЕНО кодом+тестами+доками, закоммичено+запушено (main=c4bd6bf). Зона СВОБОДНА.` Fix A concurrency 2→1 (`f442832`), Fix B profile-driven `deadline_s` 90→180 dev/eval (`24c483e`). Пороги гейта НЕ тронуты. **Валидация — GH full-eval `29183754219` на HEAD `24c483e`, IN PROGRESS** (concurrency=1 → ~45-55 мин). Кто увидит результат первым — впиши в журнал; если citation_coverage вернулся к ~1.0 И faithfulness ≥0.7 → закрыть T-041 (снять faithfulness из non_blocking в eval-thresholds.yaml, Q-16, [done] в §1 BACKLOG).
