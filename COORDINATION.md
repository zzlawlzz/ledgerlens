...
>
> ⏳ **EVAL `29181312619` ИДЁТ (dispatch 05:32 UTC, HEAD с фиксом, profile=FULL).** 41 кейс, ~8-9 faithfulness-кейсов = робастная выборка для честного закрытия T-041. **Кто увидит завершение — снимите метрики:** `gh run view 29181312619 --log | grep -A12 '"metrics"'`. Если faithfulness ≥0.7 → снять из non_blocking (Q-16), закрыть T-041.
