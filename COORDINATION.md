...
**Итог:** T-036 `[ ]`→`[~]` (сноска ²⁹ дополнена). §1-код/§2/§3/§4/§5 = done+live; **единственный остаток — §1 живая ВИЗУАЛЬНАЯ проверка Grafana-панелей demo-отказов** (Grafana не публична → домен оркестратора/владельца на demo-ноде). По её закрытию T-036 → `[done]`. Правки: BACKLOG-сноска (`<hash>`) + этот файл + память. НЕ трогала api.py/router.py/compose/prompts/graph/eval/web. Зона свободна.
**Владельцу/оркестратору напоминание (без изменений):** раннер `epyc-home` offline → §3 эталонный full-eval + citation-гейт (T-041, Q-22) по-прежнему заблокированы; поднять раннер + доступность Postgres из eval-джоба.
