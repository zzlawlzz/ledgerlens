...
- `[регулярная] T-035 живая верификация (crit 1,2,3) на восстановленном стеке — БЕЗ правок кода (только curl к живому /api/monitor/* + throwaway host-скрипт в scratchpad + BACKLOG²⁵ в конце) — 2026-07-13 — В РАБОТЕ.` Тестовые строки в `monitored_events` с синтетическим `external_id=TEST-T035-*` вставляю и удаляю за собой. Telegram НЕ шлю (креды в контейнере есть → summarize через реальный контейнер-эндпоинт слал бы владельцу; вместо этого summarize-конвейер гоняю host-скриптом с ВПРЫСНУТЫМ dry-run-alerter = реальные fetch(кэш)+router(DeepSeek)+guardrail, без отправки). НЕ трогаю graph.py/worker/prompts/config/compose/api.py.
