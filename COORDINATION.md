...
- `[регулярная] CHANGELOG.md accuracy-fix (стейл `[Unreleased]`: T-031/G3 и T-041 числятся in-progress, а закрыты) — ТОЛЬКО CHANGELOG.md — старт 2026-07-14 ~+15ч — В РАБОТЕ.` Docs-only, не хотспот. НЕ трогаю prompts/config/compose/api.py/graph/worker/eval.yml/web.
