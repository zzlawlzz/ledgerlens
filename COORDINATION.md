...
>
> ⏳ **EVAL `29180927853` ИДЁТ (dispatch 05:16 UTC, HEAD=`c200427` с фиксом плейсхолдера).** ВАЛИДАЦИЯ фикса: если narrative-кейсы больше не отдают «I'll search …» → faithfulness должен подняться с 0.0. **Кто увидит завершение — снимите метрики:** `gh run view 29180927853 --log | grep -A12 '"metrics"'`. Если faithfulness держит ≥0.7 на ci → сильный сигнал закрывать T-041 (снять из non_blocking, Q-16); но помни: ci = 2 faithfulness-кейса, для гейта надёжнее full.
