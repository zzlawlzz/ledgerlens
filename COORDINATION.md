...
- `[регулярная] T-041 root-cause фикс воркера — workers/react_worker.py + tests/unit/test_worker.py — 08:10 — В РАБОТЕ.` НЕ трогаю prompts/worker_react.md (хотспот). Нашла ПРИЧИНУ faith=0.0 на GH ci-eval `29177728775`: ReAct-воркер под нагрузкой ловит пустой ответ на synthesis-шаге → `last_ai_text` остаётся пред-тульной мыслью «I'll search X's 10-K» → она утекает как succeeded-ответ (citations есть, но текст = плейсхолдер) → judge faith 0.0. Фикс: не захватывать как ответ AIMessage, несущий tool_calls (это ReAct-action, не финальный ответ).

> ⏳ **EVAL `29177728775` ИДЁТ (dispatch 03:05 UTC на исправленном HEAD с `2cf07aa`).** Это ПЕРВЫЙ eval, гоняющий запушенный URL-фикс (прежние `29176679531`/`29176449990` падали за 2-3мин на stale origin=df8061e). Результат = валидация app-фикса T-031 + 2-я точка `citation_coverage` для Q-22 + чистая точка T-041. **Кто увидит завершение — снимите метрики:** `gh run view 29177728775 --log | grep -A12 '"metrics"'`. Если citation снова ~0.4 с ДРУГИМ набором сбойных кейсов → шум по Q-22 подтверждён.
