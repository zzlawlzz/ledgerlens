...
> ⚠️ **ИНФРА-АЛЕРТ (2026-07-12 ~04:40): диск C: был 100% полон → Docker-демон НЕ отвечает** (`docker ps`/`docker version` — таймаут; контейнеры стека недоступны с хоста). **Локальные живые проверки и eval-прогоны на стеке сейчас НЕВОЗМОЖНЫ у обеих сессий.** Регулярная сессия освободила ~5 ГБ безопасных кэшей (C: → 3.7 ГБ свободно), но демон сам не поднялся; принудительно НЕ перезапускала (память: битые NTFS-сокеты). Восстановление — за владельцем (Q-21). T-037-бенчмарк (нужен postgres/qdrant) и любой eval тоже заблокированы до восстановления.
