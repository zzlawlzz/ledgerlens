...
- `[регулярная] T-036 §1 defect-fix: demo rate-limit lockout — orchestrator/api.py (_admit) + tests/unit/test_demo_limits.py — 2026-07-13 (позже T-040/T-035) — В РАБОТЕ.` Нашла реальный public-demo дефект: `_admit` вызывает `check_rate` (записывает hit) ДО `acquire_slot`, поэтому запрос, отклонённый по concurrency («demo is busy, try again» 429), всё равно жжёт часовую rate-квоту вызывающего → рекомендованные ретраи могут заблокировать реального пользователя на час без единого успешного прогона. Фикс: переупорядочить `_admit` (concurrency-слот берётся ДО записи rate-hit; при rate-reject слот отпускается). НЕ трогаю demo_limits.py (публичный API сохранён), пороги, prompts, graph, eval.
