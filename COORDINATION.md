...
- `[регулярная] T-038 (README + финализация доков) — README.md, README.ru.md, ARCHITECTURE.md, IMPLEMENTATION_PLAN.md, CHANGELOG.md(new), docs/, BACKLOG.md — старт ~18:50 — WIP.` Тронуть НЕ хотспоты (кроме BACKLOG.md — закоммичу сразу). НЕ трогаю prompts/, config/*.yaml, docker-compose.yml, orchestrator/api.py, graph.py, eval.yml. Причина выбора: почти всё инфра-блокировано (T-036 §4 ждёт домен Q-05, T-037 local ждёт EPYC-ноду, T-041 ждёт решения владельца Q-22), а T-038 = self-contained доки. Рабочее дерево было чистое на старте (оркестратор не активен).
