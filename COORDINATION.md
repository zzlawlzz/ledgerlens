...
- `[регулярная] T-036 §4 (SSE-heartbeat для CF-прокси) — orchestrator/api.py + orchestrator/agui.py + tests — 2026-07-13 ~старт — В РАБОТЕ.` Найден реальный пробел: оба SSE-эндпоинта блокируются на `queue.get()` без таймаута → под долгим шагом оркестратора (тяжёлый RAG 60-180с) нет ни байта → CF idle-timeout (~100с) рвёт стрим. §4-крит требует «heartbeat ≤15с обязательны». Добавляю heartbeat-тик (SSE-комментарий `: keep-alive`) + `X-Accel-Buffering: no` на сами ответы (портабельно, поверх nginx). Хотспот api.py — коммичу сразу. НЕ трогаю graph.py/worker/prompts/config/docker-compose/eval.yml.
