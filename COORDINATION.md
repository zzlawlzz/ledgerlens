...
- `[регулярная] T-041 citation-блокер: фикс CPU-голодания GH-раннера (Q-22 е+ж) — .github/workflows/eval.yml, config/budgets.yaml, orchestrator/graph.py, common/agents.py — начато 2026-07-12 ~10:10.` Fix A: eval concurrency 2→1 (убирает контеншн двух jina cross-encoder rerank на 2-vCPU раннере — детерминированный корень). Fix B: worker `deadline_s` профиле-зависимый (был хардкод 90s → dev/eval 180s). НЕ трогаю пороги гейта (решение владельца). Коммичу мелко, пуш ПЕРЕД dispatch.
