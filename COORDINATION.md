...
- `[регулярная] T-039 (сайт-презентация) — site/ + .github/workflows/site.yml — старт ~19:45 — В РАБОТЕ.` Тронутые каталоги: НОВЫЙ `site/` (статический одностраничник EN+RU) + новый workflow GitHub Pages. НЕ трогаю prompts/, config/*.yaml, docker-compose.yml, orchestrator/*, web/, benchmarks/, eval.yml, deploy/. Задача self-contained, не пересекается ни с чьими хотспотами. Демо-ссылка `app.ledgerlens.space` сейчас 530 (CF-tunnel origin down = EPYC — домен владельца/оркестратора), сайт линкует на неё как есть.
