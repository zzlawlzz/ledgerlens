...
- `[регулярная] T-038 fresh-reader doc-review — README.md, README.ru.md — 2026-07-13 — В РАБОТЕ.` Только README.md/README.ru.md (не хотспоты). Сверяю фактические claims против as-built; нашла 2 stale-неточности (статус-строка «T-001…T-037 сделаны» при T-035 wip/T-036/037 [ ]; enumeration `make demo` пропускает 3 MCP-сервера + «6 сервисов»). Правлю обе, EN↔RU симметрично. НЕ трогаю prompts/config/compose/api.py/eval.
