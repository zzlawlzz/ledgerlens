...
- `[регулярная] T-040 §6: Known Limitations в README (dedicated section) — README.md, README.ru.md, docs/release/v1.0-dod.md, BACKLOG.md — старт ~19:25 2026-07-13 — WIP.` Чисто-локальный docs-слайс (не пересекается с активным T-041-eval оркестратора на EPYC). НЕ трогаю prompts/config/compose/api.py/graph/eval. Промотирую 3-пунктовый список из «Project status» в отдельную секцию «Known limitations» + добавляю недостающие релиз-ограничения (судьба RU-адаптера Q-02, глубина цен T-033 AV free-tier, исключение GPU Q-07, скоуп публичного демо), EN канон + RU зеркало.
