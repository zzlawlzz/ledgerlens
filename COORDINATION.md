...
- `[регулярная] T-041 закрытие: подтверждающий 2-й чистый full-eval на EPYC-раннере — БЕЗ правок кода, только dispatch+доки — 2026-07-14 (нач.) — В РАБОТЕ.` **КЛЮЧЕВАЯ НАХОДКА:** первый ПОЛНОСТЬЮ чистый full-eval на self-hosted EPYC-раннере `29297896263` (HEAD=`7b894cd`, после MCP-фикса) **ЗЕЛЁНЫЙ, gate exit 0, 39/41 pass:** `citation_coverage=1.0` (T-041-блокер — «потолок железа раннера» ИСЧЕЗ, как только eval пошёл чисто на EPYC), `faithfulness=0.985`, guardrail 1.0, nodata 1.0, numeric 0.923. «eval OK: no blocking thresholds violated». Это первый чистый прогон на реальном железе → citation-«потолок» был артефактом голодания GH-раннера, а не кода. Триггерю подтверждающий 2-й full-eval (плановый критерий закрытия = «2 подряд ≥0.7 на чистом раннере»). НЕ трогаю пороги до зелёного 2-го прогона. Каталоги: только COORDINATION + BACKLOG-сноска + память (+ по зелёному 2-му прогону — `config/eval-thresholds.yaml` промоут faithfulness из non_blocking). **АПДЕЙТ:** 1-й мой confirm-dispatch `29302691531` по умолчанию ушёл профилем `ci` (default eval.yml = ci; 10 кейсов, зелёный: citation 1.0/faith 1.0/gate exit 0) — но ci слабый faithfulness-сигнал, не засчитываю как 2-й FULL. Передиспетчила корректно: **full-профиль `29303631947` (`-f profile=full`) IN PROGRESS**. По его зелёному (citation≥1.0 + faith≥0.7 на 41 кейсе) закрываю T-041.
...
`[~]`: T-035, T-036 (2026-07-14: §1-код/§2/§3/§4/§5 done+live над публичным
демо, остаток = только §1 визуальная Grafana-проверка на demo-ноде), T-038,
T-039. `[ ]`: T-037, T-040. `[wip]`: T-041
