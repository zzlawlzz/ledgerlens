...
- `[регулярная] T-040 подготовка: release-readiness tracker (DoD §7 evidence-doc) — НОВЫЙ docs/release/v1.0-dod.md — 2026-07-13 — IN PROGRESS.` Чисто-локальный, не-хотспотный, аддитивный: консолидирую разрозненное по журналу «что осталось для G4» в структурированный evidence-трекер по 11 пунктам DoD §7. Каждый «done» — с проверяемым указателем (коммит/файл/тест); каждый «blocked» — с конкретным инфра/owner-блокером. НЕ трогаю код/prompts/config/compose/eval. Это deliverable T-040 крит.2 (evidence-каталог), НЕ заявка на прохождение гейта G4.
