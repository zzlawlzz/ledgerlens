...
- `[регулярная] T-036 §3/§5 ЖИВАЯ верификация против публичного демо (демо снова live после router-фикса оркестратора) — ТОЛЬКО read-only проверки + BACKLOG-сноска + COORDINATION — старт ~сейчас — В РАБОТЕ.` НЕ трогаю код/хотспоты (api.py/router.py/compose/prompts/graph/eval) — оркестратор владеет демо-зоной. Правлю ТОЛЬКО BACKLOG (T-036 сноска, коммичу сразу) + этот файл + память.
