...
- `[регулярная] T-039/T-040 site OG/canonical fix (соц-превью указывают на стейл github.io, а сайт живёт на custom-домене ledgerlens.space) — site/index.html (не хотспот) — 2026-07-14 ~+21ч — В РАБОТЕ.` НЕ трогаю prompts/config/compose/api.py/graph/worker/eval.yml/web. Живая проверка: Pages redeploy по push в `site/**` → сверю og-теги над публичным URL.
